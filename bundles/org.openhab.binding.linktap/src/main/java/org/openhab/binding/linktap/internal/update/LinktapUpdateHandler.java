 ...
 */
@NonNullByDefault
public class LinktapUpdateHandler<T> {
    ...
    private final Map<String, @Nullable Set<linktapThingDataListener<T>>> listenersMap = new ConcurrentHashMap<>();

    public boolean addListener(linktapThingDataListener<T> listener) {
        return addListener(ANY_ID, listener);
    }

    public boolean addListener(String linktapId, linktapThingDataListener<T> listener) {
        return getOrCreateListeners(linktapId).add(listener);
    }
    ...
    private Set<linktapThingDataListener<T>> getListeners(String nestId) {
        Set<linktapThingDataListener<T>> listeners = new HashSet<>();
    ...
    private Set<linktapThingDataListener<T>> getOrCreateListeners(String nestId) {
        Set<linktapThingDataListener<T>> listeners = listenersMap.get(nestId);
        if (listeners == null) {
    ...
    private void notifyListeners(String nestId, @Nullable T lastUpdate, T update) {
        Set<linktapThingDataListener<T>> listeners = getListeners(nestId);
        if (lastUpdate == null) {
    ...
    public boolean removeListener(linktapThingDataListener<T> listener) {
        return removeListener(ANY_ID, listener);
    }

    public boolean removeListener(String nestId, linktapThingDataListener<T> listener) {
